...
def combine_date_time(date_obj: datetime, time_str: str) -> pd.Timestamp:
    ...
    try:
        ...
        logger.debug("combine_date_time: parsed %s → %s", dt_str, ts)
        return ts
    except ValueError:
...
def get_level_from_room(assigned_room: str, place: str) -> str | None:
    ...
    if place == "ECEC":
        ...
        if m:
            ...
            logger.debug("get_level_from_room: ECEC %s → %s", assigned_room, lvl)
            return lvl
    ...
    if assigned_room in spellman_map:
        ...
        logger.debug("get_level_from_room: Spellman %s → %s", assigned_room, lvl)
        return lvl
    ...
    if m:
        lvl = m.group(1).title()
        lvl = lvl.replace("Toddlers", "Toddler").replace("Multi Age", "Multi-Age").replace("Pre K", "Pre-K")
        logger.debug("get_level_from_room: fallback %s → %s", assigned_room, lvl)
        return lvl
