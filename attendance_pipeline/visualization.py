...
def plot_forecast_sequence(
    ...
    header_labels = ["Date","Place","Level","Forecast Hrs","# Students","Staff Required"]
    ...
    for p in places:
        ...
        table_traces.append(go.Table(
          header=dict(values=table_cols, fill_color="paleturquoise"),
          cells =dict(values=[sub[c] for c in table_cols],
                      fill_color="lavender"),
          visible=False
        ))
    for p,l in place_level:
        sub = detailed_tbl[(detailed_tbl.Place==p)&(detailed_tbl.Level==l)]
        table_traces.append(go.Table(
          header=dict(values=table_cols, fill_color="paleturquoise"),
          cells =dict(values=[sub[c] for c in table_cols],
                      fill_color="lavender"),
    ...
    chart_traces = []
    for p,l in place_level:
        sub = detailed[(detailed.Place==p)&(detailed.Level==l)]
        chart_traces.append(go.Scatter(
            x=sub["Date"], y=sub["StaffRequired"],
            mode="lines+markers",
            name=f"{p} – {l}",
            visible=True
        ))
    ...
    offset = 1 + len(places)
    for k,(p,l) in enumerate(place_level):
        vis = [False]*(n_tbl + n_ctr)
        vis[offset + k] = True
        vis[n_tbl + k] = True
        buttons.append(dict(
